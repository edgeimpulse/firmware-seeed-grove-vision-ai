...
TfLiteStatus MicroAllocator::FinishModelAllocation(
    const Model* model, SubgraphAllocations* subgraph_allocations,
    ScratchBufferHandle** scratch_buffer_handles, uint8_t *ptr, size_t length ) {
  ...
  for (size_t subgraph_idx = 0; subgraph_idx < model->subgraphs()->size();
    ...
    TF_LITE_ENSURE_STATUS(CommitStaticMemoryPlan(
        model, subgraph_allocations[subgraph_idx].tensors,
        *scratch_buffer_handles, subgraph_idx, ptr, length) );
    TF_LITE_ENSURE_STATUS(AllocateVariables(
        subgraph, subgraph_allocations[subgraph_idx].tensors));
  }
  model_is_allocating_ = false;
...
size_t MicroAllocator::used_bytes() const {
	size_t used_head = memory_allocator_->GetHeadUsedBytes();
	size_t used_tail = memory_allocator_->GetTailUsedBytes();
	size_t used_all = memory_allocator_->GetUsedBytes();
	printf("head, tail, all = %d, %d, %d\n", used_head, used_tail, used_all );
...
TfLiteStatus MicroAllocator::CommitStaticMemoryPlan(
    const Model* model, TfLiteEvalTensor* eval_tensors,
    ScratchBufferHandle* scratch_buffer_handles, int subgraph_idx, uint8_t *ptr, size_t length) {
  size_t head_usage = 0;
  // Create static memory plan
  // 1. Calculate AllocationInfo to know the lifetime of each tensor/buffer.
  // 2. Add them into the planner (such as the GreedyMemoryPlanner).
  ...
  SimpleMemoryAllocator tmp_allocator(error_reporter_, ptr, ptr + length );
  ...
  // struct will be used by AllocationInfoBuilder to find buffer usage.
  AllocationInfo* allocation_info = reinterpret_cast<AllocationInfo*>(
		  tmp_allocator.AllocateTemp(bytes, alignof(AllocationInfo)));
  if (allocation_info == nullptr) {
    TF_LITE_REPORT_ERROR(
        error_reporter_,
        "Failed to allocate memory for allocation_info, %d bytes required",
  ...
  // Remaining arena size that memory planner can use for calculating offsets.
  size_t remaining_arena_size = tmp_allocator.GetAvailableMemory(kBufferAlignment);
  uint8_t* planner_arena = tmp_allocator.AllocateTemp(remaining_arena_size, kBufferAlignment);
  TF_LITE_ENSURE(error_reporter_, planner_arena != nullptr);
  GreedyMemoryPlanner planner(planner_arena, remaining_arena_size);
  TF_LITE_ENSURE_STATUS(CreatePlan(error_reporter_, &planner, allocation_info,
                                   allocation_info_count));
  ...
  // Reset all temp allocations used above:
  tmp_allocator.ResetTempAllocations();

  size_t actual_available_arena_size = tmp_allocator.GetAvailableMemory(kBufferAlignment);
  ...
  // memory plan in this function. Ensure that the head is set to the largest
  // memory plan sent through the allocator:
  TF_LITE_ENSURE_STATUS(tmp_allocator.SetHeadBufferSize(
      max_head_buffer_usage_, kBufferAlignment));
  return kTfLiteOk;
}
