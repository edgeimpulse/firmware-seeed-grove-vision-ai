...
#ifdef AIOT_YUNTAI_VIP
extern uint32_t _f_bss_kcf_buf;
extern uint32_t _e_bss_kcf_buf;
#endif
...
void hx_clear_bss()
{
			...
			|| ((wakeup_event & PMU_WAKEUP_CAP_CPU12_NOMOTION_ADCINT) != 0))//
	{
		...
		if(boot_flow == PMU_BOOTFLOW_FROM_RETENTION)
		{
			...
			#ifdef AIOT_YUNTAI_VIP
			kcf_buf_bss_size =  (uint32_t)&_e_bss_kcf_buf - (uint32_t)&_f_bss_kcf_buf;
			memset(&_f_bss_kcf_buf, 0, kcf_buf_bss_size);
			#endif
		}
	}else{
		bss_size = (uint32_t)&_e_bss - (uint32_t)&_f_bss;
