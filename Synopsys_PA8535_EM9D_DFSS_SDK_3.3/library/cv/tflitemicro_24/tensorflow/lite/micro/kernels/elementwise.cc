...
constexpr int kInputTensor = 0;//add shape
constexpr int kOutputTensor = 0;//add shape

void ExtractShape(const TfLiteEvalTensor* input, int32_t* output_data) {//add shape
  for (int i = 0; i < input->dims->size; ++i) {
    output_data[i] = input->dims->data[i];
  }
}
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {//add shape
  TF_LITE_ENSURE_EQ(context, NumInputs(node), 1);
  TF_LITE_ENSURE_EQ(context, NumOutputs(node), 1);

  return kTfLiteOk;
}
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {//add shape
...
TfLiteRegistration Register_SHAPE() {//add shape
  return {/*init=*/nullptr,
          /*free=*/nullptr,
          /*prepare=*/Prepare,
          /*invoke=*/Eval,
